...
**Core Message:** Người dùng có thể hỏi, tra cứu, mở tài khoản và giao dịch bằng ngôn ngữ tự nhiên — nhưng mọi hành động quan trọng đều được kiểm tra bằng Guardian Layer để ngăn truy vấn sai, giao dịch rủi ro và scam.
...
│                                                                              │
│  Example: "Tháng này tiêu nhiều quá, chuyển 5tr sang tiết kiệm"           │
...
└─────────────────────────────┬───────────────────────────────────────────────┘
                              ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  3. SPECIALIST PREPARES (không execute)                                      │
│                                                                              │
...
│    ✅ "Chi tiêu ăn uống tháng này tăng 25% so tháng trước"                │
│    ❌ "Bạn nên chuyển tiền vào sản phẩm X"                                │
...
→ Execute: return "Theo chính sách tiết kiệm (v2.1, 01/2026): ..."
→ Audit
```
...
→ SKIP Layer 2
→ Response: hard block → explain ("TK này bị 8 người report")
...
> **Nguyên tắc**: Core demo = Transaction safety + Guardian + Audit. Text2SQL/QA = priority 1 (làm khi core stable). Multi-intent = bonus.

### Priority Map

| Priority | Components | Lý do |
|----------|-----------|-------|
| **P0 — Must have** | Transaction Agent, Guardian (hard rules + anomaly + scam + risk scorer + friction), Bank confirm, Scam block, Audit trail | Đây là core differentiator. Không có = demo chết |
| **P1 — Should have** | Text2SQL Agent, SQL Validator, SQLite query execution | Đề bài yêu cầu "tra cứu bằng NL". Thiếu = bám đề yếu |
| **P2 — Nice to have** | QA RAG (keyword search), Proactive insight, Multi-intent | Thêm depth nhưng không quyết định thắng/thua |

---

### Phase 1: Scaffold + Transaction + Guardian Core (Day 1)

| Task | File(s) | Nội dung |
|------|---------|----------|
| **T1.1** Scaffold | Toàn bộ folder structure | Tạo folders, `__init__.py`, `docker-compose.yml`, `.env.example` |
...
- `POST /chat "Chuyển cho Minh 2tr"` → GREEN → confirm prompt
- `POST /chat "Chuyển 50tr ngay cho TK 0391234567"` → RED → block + explain
...
- "Tháng này tôi tiêu bao nhiêu cho ăn uống?" → SQL validated → executed → NL answer
...
**Checkpoint (Demo Day)**: `docker-compose up` → 3 scenarios liền mạch, 5–8 phút.
...
|----------|--------|-----------|
| Agent priority | Transaction first, Text2SQL second, QA last | Core demo = safety. Text2SQL = bám đề. QA = bonus |
| Guardian priority | Hard rules + anomaly Day 1, scam LLM Day 2 | Hard rules alone already demo-able |
| RAG for QA | Keyword search (no ChromaDB) | 3 docs, overkill to embed. Add ChromaDB later if needed |
...
|---|----------|----------|----------|
| 1 | "Chuyển cho Minh 2tr tiền ăn" | GREEN → preview → confirm → success + nudge | P0 |
| 2 | "Chuyển ngay 50tr vào TK 0391234567" | RED → block → explain → suggest hotline | P0 |
| 3 | Audit trail visible | Expandable decision log per interaction | P0 |
| 4 | "Tháng này tôi tiêu bao nhiêu cho ăn uống?" | SQL → validate → execute → NL answer | P1 |
| 5 | "Lãi suất tiết kiệm 6 tháng là bao nhiêu?" | Answer + citation + policy version | P2 |
