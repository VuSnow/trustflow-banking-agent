"""
Load mock_data CSV files into PostgreSQL database.
Usage: python load_csv_to_postgres.py

Environment variables:
  DATABASE_URL  - PostgreSQL connection string (default: postgresql://postgres:postgres@localhost:5432/banking)

Or individual vars:
  PGHOST     (default: localhost)
  PGPORT     (default: 5432)
  PGDATABASE (default: banking)
...
# JSONB fields (store as raw JSON string, PostgreSQL handles casting)
...
def convert_value(col, val):
    ...
    if col in JSON_FIELDS:
        # Return as string - psycopg2 will handle JSONB casting
        return val
    return val
...
def load_csv(conn, table_name):
    ...
    values = []
    for row in rows:
        row_values = tuple(convert_value(col, row[col]) for col in columns)
