...
def format_value(col, val):
    ...
    # Default: string/timestamp/uuid → quoted
    return f"'{escape_sql_string(val)}'"


def generate_inserts(table_name):
    """Generate INSERT statements for a table from its CSV."""
    ...
    # Join with commas, last one gets semicolon
    lines.append(",\n".join(value_lines) + ";\n")
...
def main():
    ...
    # 2. Write individual table data files
    ...
    for i, table in enumerate(TABLES, start=1):
        ...
        file_num = f"{i + 1:02d}"  # 02 onwards (01 is schema)
    ...
    with open(all_path, "w", encoding="utf-8") as f:
        ...
        f.write("-- Run: psql -d banking -f all_in_one.sql\n")
        f.write("-- ============================================================\n\n")
    ...
    print(f"\nUsage:")
    print(f"  # All at once:")
    print(f"  psql -d banking -f {SQL_DIR}/all_in_one.sql")
    print(f"  # Or step by step:")
    print(f"  psql -d banking -f {SQL_DIR}/01_schema.sql")
    print(f"  psql -d banking -f {SQL_DIR}/02_customers.sql")
    print(f"  psql -d banking -f {SQL_DIR}/03_accounts.sql")
    print(f"  # ... etc")
